                    ...
                    <form onSubmit={cadastrarAgendamento} className={styles.formulario_servico}>
                        <div className={`${styles.div_profissionais_servico}`}>
                            <div className="d-flex flex-column w-50">
                                ...
                                <select className="input" value={idServico}
                                        onChange={(e) => setIdServico(Number(e.target.value))}>
                                    <option>Selecione os serviços</option>
                                    {servicos?.map((servico) => (
                                        <option value={servico.id_servico}
                                                key={servico.id_servico}>{servico.nome}</option>
                                    ))}
                                </select>
                            </div>
                            <div className="d-flex flex-column w-50">
                                <label>Selecione os profissionais</label>
                                <select className="input" value={idProfissional}
                                    ...
                                    {profissionaisFiltrados?.map((profissional) => (
                                        <option value={profissional.id_profissional}
                                                key={profissional.id_profissional}>{profissional.nome}</option>
                                    ))}
                                </select>
                            </div>
                        </div>
                        ...
                        <div className={styles.div_observacoes}>
                            ...
                            <input type="text" value={observacao} onChange={event => setObservacao(event.target.value)}
                                   placeholder="Escreva as observações a seguir..."/>
                        </div>
